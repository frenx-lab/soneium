...
# 自动获取链上gas费
gas_price = web3.eth.gas_price  # 默认的Gas价格（以wei为单位）
max_fee = int(gas_price * 1.2)  # 为maxFeePerGas设置稍高的价格
...
def send_transaction(to, value, data, private_key):
    ...
    tx = {
        ...
        'to': to,
        'value': value,
        'maxFeePerGas': max_fee,  # 使用自动计算的gas费用
...
def main():
    # 自定义交易金额和次数
    eth_amount = 0.0001  # 每次交易的ETH数量
    num_transactions = 1  # 每个地址的交易次数
    ...
    for private_key in private_keys:
        ...
        required_eth = eth_amount + (GAS_LIMIT * max_fee / 10**18)  # ETH amount + max gas cost
